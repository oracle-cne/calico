...
for IMAGE in ${CALICO_IMAGE}; do
	...
	echo "build-image.sh: saving image=${REGISTRY}/${IMAGE}:${VERSION} to ${IMAGE_LOCATION}/oracle_docker/${IMAGE}.tar"
	podman save -o "${IMAGE_LOCATION}/oracle_docker/${IMAGE}.tar" "${REGISTRY}/${IMAGE}:${VERSION}"
done
